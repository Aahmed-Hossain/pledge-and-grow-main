...
//= Data
import data from '@/data/Maintenance/services.json';

function Services({ lightMode }) {


  const [height, setHeight] = useState('13rem'); // Default for PC
  ...
  useEffect(() => {
    ...
    const updateHeight = () => {
      if (typeof window !== 'undefined') {
        ...
        if (screenWidth <= 480) {
          setHeight('20rem'); // Mobile
        } else if (screenWidth <= 768) {
          setHeight('23rem'); // Tablet
        } else {
          setHeight('13rem'); // PC
        }
      }
          ...
          <div className="row">
            <div className="col-lg-7">
              <div className="position-re">
                ...
                <h2 className="fz-50">Nos Services de Maintenance</h2>
              </div>
            </div>
        ...
        <div style={{display: 'flex', flexDirection: 'row-reverse' , alignItems: 'center', alignContent: 'center' ,justifyContent: 'center'}} className="row">
          {
            data.map((item, index) => (
              <div  className="col-lg-6" key={item.id}>
                <div style={{ height }} className={`serv-item d-flex ${index !== data.length - 1 ? 'mb-30' : ''} radius-10`}>
                  <div style={{width: '7rem'}} className="">
